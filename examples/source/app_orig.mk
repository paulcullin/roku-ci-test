...
pkg: install
	...
	@if [ "$(HTTPSTATUS)" == " 401" ]; \
	then \
		curl --user $(USERPASS) --digest -s -S -Fmysubmit=Package -Fapp_name=$(APPNAME)/$(VERSION) -Fpasswd=$(DEVKEY) -Fpkg_time=`expr \`date +%s\` \* 1000` "http://$(ROKU_DEV_TARGET)/plugin_package" | grep '^<tr><td><font face="Courier"><a' | sed 's/.*href=\"\([^\"]*\)\".*/\1/' | sed 's#pkgs//##' | xargs -I curl --user $(USERPASS) --digest -s -S -o $(PKGREL)/$(APPNAME)_$(VERSION).pkg http://$(ROKU_DEV_TARGET)/pkgs/{} ; \
	else \
		curl -s -S -Fmysubmit=Package -Fapp_name=$(APPNAME)/$(VERSION) -Fpasswd=$(DEVKEY) -Fpkg_time=`expr \`date +%s\` \* 1000` "http://$(ROKU_DEV_TARGET)/plugin_package" | grep '^<tr><td><font face="Courier"><a' | sed 's/.*href=\"\([^\"]*\)\".*/\1/' | sed 's#pkgs//##' | xargs -I curl -s -S -o $(PKGREL)/$(APPNAME)_$(VERSION).pkg http://$(ROKU_DEV_TARGET)/pkgs/{} ; \
	fi
